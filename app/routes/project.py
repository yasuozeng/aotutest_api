...
from base import project_base
from flask import Flask, jsonify, request
from base import mysql_base,app_base
...
# 定义路由，创建接口
@project_bp.route('/project_list', methods=['GET'])                    #用于创建路由的装饰器
def project_list():
    #查询项目名字列表
...
@project_bp.route('/project_add', methods=['POST'])                    #用于创建路由的装饰器
def project_add():
    #新增项目
    # 获取 POST 请求的 JSON 数据
    data = request.json
...
@project_bp.route('/project_delete', methods=['POST'])                    #用于创建路由的装饰器
def project_delete():
    #查询项目名字列表
    # 获取 POST 请求的 JSON 数据
    data = request.json
...
def project_datails():
    ...
    data = request.json
    return data
    # app_base.select_project_id_name(project_name=data)
